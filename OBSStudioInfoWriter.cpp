...
bool obstudio_infowriter_syncnameandpathwithvideo_property_modified(obs_properties_t *props,
								    ...
								    obs_data_t *settings)
{
	obs_property_t *prop_setting_file = obs_properties_get(props, setting_file);
	const bool previously_enabled = obs_property_enabled(prop_setting_file);
	const bool sync_selected = obs_data_get_bool(settings, setting_syncnameandpathwithvideo);

	obs_property_set_enabled(prop_setting_file, !sync_selected);

	obs_property_t *prop_automaticoutputextension = obs_properties_get(props, setting_automaticoutputextension);
...
void *obstudio_infowriter_create(obs_data_t *settings, obs_source_t *source)
{
	...
	obs_hotkey_register_source(source, "InfoWriter.Hotkey10", "Hotkey 10", obstudio_infowriter_write_hotkey10,
				   Writer);
	obs_hotkey_register_source(source, "InfoWriter.Hotkey11", "Hotkey 11", obstudio_infowriter_write_hotkey11,
				   Writer);
	obs_hotkey_register_source(source, "InfoWriter.Hotkey12", "Hotkey 12", obstudio_infowriter_write_hotkey12,
				   Writer);
	obs_hotkey_register_source(source, "InfoWriter.Hotkey13", "Hotkey 13", obstudio_infowriter_write_hotkey13,
				   Writer);
	obs_hotkey_register_source(source, "InfoWriter.Hotkey14", "Hotkey 14", obstudio_infowriter_write_hotkey14,
				   Writer);
...
obs_properties_t *obstudio_infowriter_properties(void *unused)
{
	...
	auto list = obs_properties_add_list(props, setting_outputformat, obs_module_text("Output format"),
	...
	obs_properties_add_bool(props, setting_shouldlogstreaming,
				obs_module_text("Log Streaming events (applies to default output)"));
	obs_properties_add_bool(props, setting_shouldlogabsolutetime,
				obs_module_text("Log intermediate absolute times (applies to default output)"));
	obs_properties_add_bool(props, setting_shouldloghotkeyspecifics,
				obs_module_text("Log hotkey specifics (applies to default output)"));
...
void obstudio_infowriter_get_defaults(obs_data_t *settings)
{
	...
	obs_data_set_default_bool(settings, setting_syncnameandpathwithvideo, false);
...
void obstudio_infowriter_update(void *data, obs_data_t *settings)
{
	...
	const char *automaticextension = obs_data_get_string(settings, setting_automaticoutputextension);
